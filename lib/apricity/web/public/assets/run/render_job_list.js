...
function statusIcon(status) {
  // Colored dot indicators matching DAG style
  return {
    running: "🟡",
    success: "🟢",
    failure: "🔴",
        ...
        ([nodeId, job]) => {
          ...
          return `<li>
              <a href="#${encodeURIComponent(nodeId)}" 
                 class="flex items-center justify-between px-3 py-2 rounded-lg transition-colors ${isActive ? 'bg-blue-100 text-blue-700 font-medium' : 'hover:bg-gray-100 text-gray-600'}">
                <div class="flex items-center space-x-2 min-w-0">
                  <span class="text-base flex-shrink-0">${statusIcon(job.status)}</span>
                  <div class="flex flex-col min-w-0">
                    <span class="text-sm truncate">${displayName}</span>
                    ${matrixBadge}
                  </div>
                </div>
                ${jobDuration ? `<span class="text-xs text-gray-400 font-mono flex-shrink-0">${jobDuration}</span>` : ''}
              </a>
            </li>`;
        }
      )
