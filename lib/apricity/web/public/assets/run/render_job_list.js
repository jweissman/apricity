          ...
          return `<li>
              <a href="#${encodeURIComponent(nodeId)}" 
                ...
                <div class="flex items-center space-x-3 min-w-0">
                  <span class="text-sm flex-shrink-0 ${statusColor}">${statusIcon(job.status)}</span>
...
function renderJob(job, nodeId, dagNodes) {
  ...
  const stepsHtml = hasSteps 
    ? Object.entries(job.steps).map(([name, step]) => {
        ...
        return `
          <div class="rounded-2xl border border-slate-200/60 overflow-hidden transition-all duration-300 ${isPending ? 'opacity-40' : ''} ${isRunning ? 'ring-2 ring-amber-200/60 ring-offset-2' : ''} hover:border-slate-300/80">
            <div 
              class="flex items-center justify-between px-5 py-4 ${isPending ? 'bg-slate-50/30' : 'bg-white'} ${!isPending ? 'cursor-pointer hover:bg-slate-50/50' : ''} transition-colors duration-200"
              ${!isPending ? `onclick="toggleStepOutput('${stepId}')"` : ''}
            >
              <div class="flex items-center space-x-3.5">
                <span class="text-sm ${statusColor}">${isRunning ? '<i class="fas fa-circle-notch fa-spin text-amber-400"></i>' : statusIcon(step.status)}</span>
                <span class="text-sm font-medium ${isPending ? 'text-slate-400' : 'text-slate-700'}">${name}</span>
                ${step.duration ? `<span class="text-xs text-slate-400 font-mono tabular-nums bg-slate-100/80 px-2.5 py-1 rounded-lg">${formatDuration(step.duration)}</span>` : ''}
              </div>
            ...
            ${!isPending && hasOutput ? `
              <div id="step-${name}-output" class="step-output ${isOpen ? "" : "hidden"}">
                ...
                ` : ''}
                <div class="bg-gradient-to-b from-slate-900 to-slate-950 p-5 terminal-output max-h-96 overflow-auto">
                  <pre class="stdout text-slate-300 whitespace-pre-wrap break-all text-xs leading-relaxed">${escapeHtml(step.output.stdout) || '<span class="text-slate-500 italic">No output yet</span>'}</pre>
                  ${step.output.stderr ? `<pre class="stderr text-rose-300 whitespace-pre-wrap break-all mt-4 text-xs leading-relaxed pt-4 border-t border-slate-800">${escapeHtml(step.output.stderr)}</pre>` : ''}
                </div>
              </div>
            ` : ''}
          </div>`;
