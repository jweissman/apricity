...
function statusBadge(status) {
  // Cleaner, more refined badge styles - subtle rings, no icons
  const configs = {
    running: {
      bg: "bg-white",
      text: "text-amber-700",
      ring: "ring-1 ring-amber-300",
      dot: '<span class="w-1.5 h-1.5 rounded-full bg-amber-500 animate-pulse"></span>',
      label: "Running"
    },
    success: {
      bg: "bg-emerald-50",
    ...
    failure: {
      ...
      dot: '<span class="w-1.5 h-1.5 rounded-full bg-red-500"></span>',
      label: "Failed"
    },
    pending: {
      bg: "bg-frost-50",
...
function formatJobMetadata(job) {
  if (!job.metadata || Object.keys(job.metadata).length === 0) {
    console.warn('No job metadata available');
    return '';
  }

  console.debug('Formatting job metadata:', job.metadata);
  ...
  for (const [stepName, step] of Object.entries(job.steps)) {
    ...
    if (stepEl) {
      ...
      if (statusSpan) {
        ...
        statusSpan.className = `w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0 step-status-icon ${isRunning ? 'bg-white' : isSuccess ? 'bg-emerald-100' : isFailure ? 'bg-red-100' : 'bg-frost-100'}`;
        
        // Update icon HTML to match initial render
        if (isRunning) {
          statusSpan.innerHTML = '<i class="fas fa-circle-notch fa-spin text-amber-600"></i>';
        } else if (isSuccess) {
          statusSpan.innerHTML = '<i class="fas fa-check text-emerald-600 text-xs"></i>';
        } else if (isFailure) {
          statusSpan.innerHTML = '<i class="fas fa-times text-red-500 text-xs"></i>';
        } else if (isPending) {
          statusSpan.innerHTML = '<i class="far fa-circle text-frost-400 text-xs"></i>';
        } else {
          statusSpan.innerHTML = statusIcon(step.status);
          statusSpan.className += ` text-sm ${statusIconColor(step.status)}`;
        }
      }
      ...
      if (durationSpan) {
        if (step.duration != null) {
          ...
          delete durationSpan.dataset.startedAt;   // ✅ important
        } else if (isRunning && step.startTime != null) {
          durationSpan.dataset.startedAt = String(step.startTime); // ✅ store ms string
          durationSpan.classList.remove('hidden');
          // DON'T set text here if you want timer to be sole writer
        } else {
...
function renderJob(job, nodeId, dagNodes) {
  ...
  const stepsHtml = hasSteps
    ? Object.entries(job.steps).map(([name, step]) => {
      ...
      // Status icon with better visual hierarchy
      let statusIconHtml;
      if (isRunning) {
        statusIconHtml = '<i class="fas fa-circle-notch fa-spin text-amber-600"></i>';
      } else if (isSuccess) {
        statusIconHtml = '<i class="fas fa-check text-emerald-600 text-xs"></i>';
      } else if (isFailure) {
        statusIconHtml = '<i class="fas fa-times text-red-500 text-xs"></i>';
      } else if (isPending) {
        statusIconHtml = '<i class="far fa-circle text-frost-400 text-xs"></i>';
      } else {
        statusIconHtml = statusIcon(step.status);
      }

      return `
          <div class="group rounded-md border transition-all ${isPending ? 'border-frost-200 opacity-50' : isRunning ? 'border-amber-200 bg-amber-50/30' : 'border-frost-200 hover:border-frost-300 bg-white'}" data-step-card="${stepId}">
            <button 
              type="button"
              class="step-header w-full flex items-center justify-between px-5 py-3.5 text-left transition-colors rounded-md ${isPending ? 'bg-frost-50 cursor-default' : 'hover:bg-frost-50/50'}"
            ...
            >
              <div class="flex items-center gap-3 min-w-0">
                <span class="w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0 ${isRunning ? 'bg-white' : isSuccess ? 'bg-emerald-100' : isFailure ? 'bg-red-100' : 'bg-frost-100'} step-status-icon">
                  ${statusIconHtml}
                </span>
                <span class="step-name text-sm font-medium ${isPending ? 'text-frost-400' : 'text-frost-800'} truncate">${name}</span>
            ...
            ${!isPending && hasOutput ? `
              <div id="step-${stepId}-output" class="step-output ${isOpen ? "" : "hidden"} border-t border-frost-100" role="region">
                ${step.status === 'failure' && step.error ? `
                  <div class="bg-red-50 border-l-4 border-red-400 px-5 py-4">
                    <div class="flex items-start gap-3">
                      ...
                      <div class="min-w-0">
                        <h5 class="text-sm font-semibold text-red-700 mb-1">Error</h5>
                        <p class="text-sm text-red-600 font-mono break-all">${escapeHtml(step.error)}</p>
                      </div>
                    </div>
                  </div>
                ` : ''}
...
window.copyToClipboard = function(text, button) {
  navigator.clipboard.writeText(text).then(() => {
    ...
    button.innerHTML = '<i class="fas fa-check text-emerald-600 text-xs"></i><span class="text-emerald-600">Copied!</span>';
