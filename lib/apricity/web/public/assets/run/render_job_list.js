...
export function renderJobList(run) {
  ...
  console.debug(`[RenderJobList] needsFullRender=${needsFullRender}, lastJobId=${lastRenderedJobId}, selectedJobId=${selectedJobId}`);
  ...
  const hadTracking = stepOutputLengths.size > 0;
  console.log(`[RenderJobList] Full render - clearing ${stepOutputLengths.size} tracked outputs`);
    ...
    for (const [stepName, step] of Object.entries(activeJob.steps || {})) {
      ...
      if (step.output) {
        ...
        if (stdoutLen > 0 || stderrLen > 0) {
          console.log(`[Render] Tracked output for ${stepId}: stdout=${stdoutLen}b, stderr=${stderrLen}b`);
        } else {
...
function hasStructuralChanges(oldSnapshot, newSnapshot, selectedJobId) {
  ...
  for (const stepName of newStepNames) {
    ...
    if (!oldStep.hasOutput && newStep.hasOutput) {
      console.debug(`[Structural] Step "${stepName}" now has output, triggering full render`);
      return true;
    }
  }
    ...
    if (step.output.stdout) {
      ...
      if (stdoutEl) {
        ...
        if (currentTrackedLength === undefined) {
          ...
          console.debug(`[UpdateTerminal] Initializing stdout tracking for ${stepId}: ${fullStdout.length}b (no prior tracking)`);
        ...
        if (fullStdout.length > lastLen) {
          ...
          console.debug(`[UpdateTerminal] Appending ${newChunk.length} chars to stdout-${stepId} (${lastLen} → ${fullStdout.length})`);
...
function renderJob(job, nodeId, dagNodes) {
  ...
  const stepsHtml = hasSteps
    ? Object.entries(job.steps).map(([name, step]) => {
      ...
      // Always show output containers for running steps (for live streaming)
      // or completed steps that have output
      const shouldShowOutputSection = !isPending && (hasOutput || isRunning);
      ...
      return `
          <div class="group ${stepCardClasses}" data-step-card="${stepId}">
            <button 
              ...
              ${shouldShowOutputSection ? `onclick="toggleStepOutput('${stepId}')"` : ''}
              ${shouldShowOutputSection ? `aria-expanded="${isOpen}" aria-controls="step-${stepId}-output"` : ''}
            >
              ...
              <div class="flex items-center gap-2 flex-shrink-0">
                ${shouldShowOutputSection ? `<i class="fas fa-chevron-down text-frost-400 text-xs transition-transform ${isOpen ? 'rotate-180' : ''} group-hover:text-frost-600"></i>` : ''}
              </div>
            </button>
