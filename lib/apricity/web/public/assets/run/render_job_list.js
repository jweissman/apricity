  ...
  for (const [stepName, step] of Object.entries(job.steps)) {
    ...
    if (stepEl) {
      ...
      if (durationSpan) {
        ...
        // } else if (isRunning && step.startTime) {
        //   durationSpan.dataset.startedAt = step.startTime;
        //   const durationSeconds = (Date.now() - new Date(step.startTime).getTime()) / 1000;
          ...
          <div class="rounded-lg border border-frost-200 overflow-hidden ${isPending ? 'opacity-50' : ''} ${isRunning ? 'ring-1 ring-amber-300' : ''}" data-step-card="${stepId}">
            ...
            >
              <div class="flex items-center gap-3">
                ...
                <span class="step-duration text-xs text-frost-500 font-mono tabular-nums ${step.duration || isRunning ? '' : 'hidden'}" data-step-duration="${stepId}" ${isRunning && step.startTime ? `data-started-at="${step.startTime}"` : ''}>${step.duration ? formatDuration(step.duration) : ''}</span>
              </div>
                  ...
                  <div class="flex items-center gap-2 flex-shrink-0">
                    ...
                    ${isJUnit ? `
                      <a 
                        href="${viewUrl}" 
                        target="_blank"
                        class="inline-flex items-center px-2 py-1 text-xs font-medium text-white bg-amber-600 hover:bg-amber-700 rounded"
