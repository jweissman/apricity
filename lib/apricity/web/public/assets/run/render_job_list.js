...
export function renderJobList(run) {
  ...
  jobSidebar.className = "job-sidebar shrink-0 w-64 border-r border-gray-300 pr-4";
    ...
    <ul class="space-y-2">
      ...
      .map(
        ([nodeId, job]) => {
          ...
          return `<li>
              <a href="#${nodeId}" 
                 class="flex items-center space-x-3 px-3 py-2 rounded-lg transition-colors ${isActive ? 'bg-blue-600/20 text-blue-400 border border-blue-500/30' : 'hover:bg-gray-100 text-gray-700 hover:text-gray-900'}">
      ...
      .map(([name, step]) => {
        ...
        return `
          <div class="rounded-lg border border-gray-700 overflow-hidden">
            <div 
              class="flex items-center justify-between px-4 py-3 bg-gray-800/50 cursor-pointer"
