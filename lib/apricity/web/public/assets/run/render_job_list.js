...
function statusBadge(status) {
  ...
  const configs = {
    ...
    success: {
      ...
      icon: '<i class="fas fa-check text-emerald-500 text-[10px]"></i>',
      label: "Passed"
    },
    failure: {
      bg: "bg-rose-50",
      ...
      icon: '<i class="fas fa-times text-rose-500 text-[10px]"></i>',
      label: "Failed"
    },
    pending: {
      bg: "bg-slate-50",
      ...
      icon: '<span class="w-2 h-2 rounded-full bg-slate-300"></span>',
      label: "Pending"
    }
  };
...
export function renderJobList(run) {
  ...
  jobSidebar.className = "job-sidebar shrink-0 w-56 border-r border-slate-200 pr-4";
  jobSidebar.id = "job-sidebar";
  jobSidebar.innerHTML = `
    <div class="flex items-center justify-between mb-3">
      <h3 class="text-xs font-medium text-slate-500 uppercase tracking-wide flex items-center gap-1.5">
        ...
        ([nodeId, job]) => {
          ...
          return `<li>
              <a href="#${encodeURIComponent(nodeId)}" 
                 class="flex items-center justify-between px-3 py-2 rounded-lg ${isActive ? 'bg-sky-50 text-sky-700 font-medium' : 'hover:bg-slate-50 text-slate-600'}">
                ...
                ${jobDuration ? `<span class="text-xs text-slate-400 font-mono flex-shrink-0 tabular-nums">${jobDuration}</span>` : ''}
              </a>
            </li>`;
        }
      )
...
function updateDifferential(run, selectedJobId, dagNodes) {
  ...
  for (const [stepName, step] of Object.entries(job.steps)) {
    ...
    if (stepEl) {
      ...
      if (headerEl) {
        headerEl.classList.toggle('bg-slate-50', isPending);
        headerEl.classList.toggle('bg-white', !isPending);
        headerEl.classList.toggle('cursor-pointer', !isPending);
        headerEl.classList.toggle('hover:bg-slate-50', !isPending);
      }
      ...
      if (statusSpan) {
        ...
        statusSpan.innerHTML = isRunning ? '<i class="fas fa-circle-notch fa-spin text-amber-500"></i>' : statusIcon(step.status);
      }
...
function renderJob(job, nodeId, dagNodes) {
  ...
  const stepsHtml = hasSteps 
    ? Object.entries(job.steps).map(([name, step]) => {
        ...
        return `
          <div class="rounded-lg border border-slate-200 overflow-hidden ${isPending ? 'opacity-50' : ''} ${isRunning ? 'ring-1 ring-amber-300' : ''}" data-step-card="${stepId}">
            <button 
              type="button"
              class="step-header w-full flex items-center justify-between px-4 py-3 text-left ${isPending ? 'bg-slate-50' : 'bg-white'} ${!isPending ? 'cursor-pointer hover:bg-slate-50' : ''}"
              ${!isPending ? `onclick="toggleStepOutput('${stepId}')"` : ''}
              ${!isPending && hasOutput ? `aria-expanded="${isOpen}" aria-controls="step-${stepId}-output"` : ''}
            >
              <div class="flex items-center gap-3">
                <span class="step-status-icon text-sm ${statusColor}">${isRunning ? '<i class="fas fa-circle-notch fa-spin text-amber-500"></i>' : statusIcon(step.status)}</span>
                <span class="step-name text-sm font-medium ${isPending ? 'text-slate-400' : 'text-slate-700'}">${name}</span>
                <span class="step-duration text-xs text-slate-500 font-mono tabular-nums ${step.duration || isRunning ? '' : 'hidden'}" data-step-duration="${stepId}" ${isRunning && step.started_at ? `data-started-at="${step.started_at}"` : ''}>${step.duration ? formatDuration(step.duration) : ''}</span>
              </div>
              ${!isPending && hasOutput ? `<i class="fas fa-chevron-${isOpen ? 'up' : 'down'} text-slate-400 text-xs"></i>` : ''}
            </button>
            ${!isPending && hasOutput ? `
                  ...
                  <div class="bg-rose-50 border-l-4 border-rose-400 p-4">
                    <div class="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <p class="text-sm text-rose-600 font-mono mt-1">${escapeHtml(step.error)}</p>
                      </div>
                    </div>
                  </div>
                ` : ''}
                <div class="bg-slate-900 p-4 terminal-output max-h-96 overflow-y-auto overflow-x-hidden">
                  ...
                  <pre id="stderr-${stepId}" class="stderr whitespace-pre-wrap break-words mt-3 text-xs leading-relaxed pt-3 border-t border-slate-700 overflow-hidden ${step.output.stderr ? '' : 'hidden'}">${renderAnsi(step.output.stderr)}</pre>
                </div>
              </div>
            ` : ''}
          </div>`;
    ...
    : `<div class="text-center py-12 text-slate-400">
        <i class="fas fa-hourglass-start text-2xl text-slate-300 mb-3"></i>
        <p class="text-sm text-slate-500">Waiting to start...</p>
      </div>`;
  ...
  jobEl.innerHTML = `
    <div class="space-y-3">
      <div class="flex items-center justify-between pb-2 border-b border-slate-100">
        <div>
          <h3 id="${nodeId}" class="text-base font-semibold text-slate-800">${displayName}</h3>
          ${matrixInfo}
        </div>
      ...
      ${job.artifacts && Object.keys(job.artifacts).length > 0 ? `
        <div class="mt-3 p-3 rounded-lg bg-slate-50 border border-slate-200">
          <h4 class="text-xs font-medium text-slate-600 uppercase tracking-wide mb-2 flex items-center gap-2">
              ...
              .map(([key, path]) => {
                ...
                <div class="flex items-center justify-between p-2 bg-white rounded border border-slate-200">
                  <div class="flex items-center gap-2 min-w-0">
                    ...
                    <div class="min-w-0">
                      <div class="text-sm font-medium text-slate-700">${key}</div>
                  ...
                  <div class="flex items-center gap-2 flex-shrink-0">
                    ...
                    <a 
                      href="${downloadUrl}" 
                      download
                      class="inline-flex items-center px-2 py-1 text-xs font-medium text-slate-700 bg-slate-100 hover:bg-slate-200 rounded"
              ...
              .map(([key, elements]) => `
                <div class="flex items-center justify-between p-2 bg-white rounded border border-slate-200">
                  <div class="flex items-center gap-2 min-w-0">
                    ...
                    <div class="min-w-0">
                      <div class="text-sm font-medium text-slate-700">${humanize(key)}</div>
