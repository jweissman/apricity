...
const STATUS_STYLES = {
  running: "fill:#fef3c7,stroke:#f59e0b,stroke-width:2.5px",
  success: "fill:#ecfdf5,stroke:#10b981,stroke-width:2.5px",
  failure: "fill:#fef2f2,stroke:#ef4444,stroke-width:2.5px",
  skipped: "fill:#f8fafc,stroke:#94a3b8,stroke-width:2px",
  pending: "fill:#fafafa,stroke:#e2e8f0,stroke-width:2px,stroke-dasharray:5 5"
};
...
export function generateMermaidSyntax(dag, jobs) {
  ...
  dag.nodes.forEach((node) => {
    ...
    const fullLabel = statusIndicator ? `${statusIndicator} ${label}` : label;
    
    // Use stadium shape for wide horizontal nodes
    lines.push(`  ${safe}([${fullLabel}])`);
